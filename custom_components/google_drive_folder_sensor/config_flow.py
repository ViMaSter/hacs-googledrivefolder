"""Config flow for the Google Drive Folder integration."""
...
from .api import AsyncConfigFlowAuth, DriveFolderClient
...
class OAuth2FlowHandler(
    ...
    """Config flow to handle Google Drive Folder OAuth2 authentication."""
    ...
    async def async_oauth_create_entry(self, data: dict[str, Any]) -> ConfigFlowResult:
        ...
        try:
            ...
            if not isinstance(folder_contents, list):
                raise self.async_abort(
                    reason="unexpected_response",
                    description_placeholders={
                        "message": "Expected a list of folder contents"
                    },
                )
            self.logger.debug("Number of items in folder: %d", len(folder_contents))
        except GoogleDriveApiError as err:
        ...
        return self.async_create_entry(
            ...
            description_placeholders={
                "folder_name": "root",
                "url": f"https://drive.google.com/drive/",
            },
        )
